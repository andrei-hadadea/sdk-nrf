			      ...
			      size_t password_length, const psa_pake_cipher_suite_t *cipher_suite)
{
	...
	return set_password_key(operation, password, password_length);
}
							  ...
							  size_t *output_length)
{
	...
	sx_status = sx_mod_primitive_cmd(&req, cmd_add, &modulo, &c_kv, &c_temp_b,
					      &result);
	status = silex_statuscodes_to_psa(sx_status);
	if (status != PSA_SUCCESS) {
		goto error;
						  ...
						  const size_t S_length)
{
	...
	sx_status = sx_mod_primitive_cmd(&req, cmd_mul, &modulo, &k, &c_temp_g, &kg);
	status = silex_statuscodes_to_psa(sx_status);
	if (status != PSA_SUCCESS) {
		goto exit;
	}
	...
	sx_status = sx_mod_primitive_cmd(&req, cmd_sub, &modulo, &B, &c_kg, &temp_b);
	status = silex_statuscodes_to_psa(sx_status);
	if (status != PSA_SUCCESS) {
		goto exit;
	}
	...
	sx_status = sx_mod_primitive_cmd(&req, cmd_mul, &modulo, &c_temp_1, &c_temp_3,
					      &result);
	status = silex_statuscodes_to_psa(sx_status);
exit:
	sx_pk_release_req(&req);
						  ...
						  const size_t S_length)
{
	...
	sx_get_const_op(&temp_1, &c_temp_1);
	sx_status = sx_mod_primitive_cmd(&req, cmd_mul, &modulo, &A, &c_temp_1,
					      &temp_2);
	status = silex_statuscodes_to_psa(sx_status);
	if (status != PSA_SUCCESS) {
		goto exit;
